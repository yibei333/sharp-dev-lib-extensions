...
namespace SharpDevLib.Extensions.Jwt.Tests;
...
public class JwtServiceTests
{
    ...
    public JwtServiceTests()
    {
        ...
        _rsaKey = serviceProvider.GetRequiredService<IRsaKey>();
    }

    [TestMethod]
    public void JwtHS256Test()
    {
        ...
        var result = _jwtService.Create(new JwtCreateOption(JwtAlgorithm.HS256, key, payload));
        Assert.AreEqual(expected, result);
    }

    [TestMethod]
    public void JwtHS256VerifyTest()
    {
        ...
        var key = "12345678901234567890123456789012";
        var result = _jwtService.Verify(new JwtVerifyOption(token, key));
        Console.WriteLine(result.Payload);
        Assert.IsTrue(result.IsVerified);
    }

    [TestMethod]
    public void JwtRS256Test()
    {
...
-----END RSA PRIVATE KEY-----";
        var pkcs8Key = _rsaKey.ConvertType(key, RsaKeyType.Pkcs8);
        ...
        var result = _jwtService.Create(new JwtCreateOption(JwtAlgorithm.RS256, pkcs8Key, payload));
        Assert.AreEqual(expected, result);
    }

    [TestMethod]
    public void JwtRS256VerifyTest()
    {
