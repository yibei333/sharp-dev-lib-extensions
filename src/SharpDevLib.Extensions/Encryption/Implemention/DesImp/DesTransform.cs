...
namespace SharpDevLib.Extensions.Encryption.Implemention.DesImp;

internal class DesTransform : IEncryptionTransform<DesEncryptOption, DesDecryptOption>
{
    public byte[] Decrypt(string data, DesDecryptOption option)
    {
        if (data.IsNull()) throw new ArgumentNullException(nameof(data));
        return Decrypt(Convert.FromBase64String(data), option);
    }

    private byte[] Decrypt(byte[] data, DesDecryptOption option)
    {
    ...
    public void DecryptFile(string filename, string decryptedFileName, DesDecryptOption option)
    {
    ...
    public void DecryptFile(string filename, Stream decryptedStream, DesDecryptOption option)
    {
    ...
    public byte[] Encrypt(string data, DesEncryptOption option)
    {
        if (data.IsNull()) throw new ArgumentNullException(nameof(data));
        return Encrypt(Encoding.UTF8.GetBytes(data), option);
    }

    private byte[] Encrypt(byte[] data, DesEncryptOption option)
    {
    ...
    public void EncryptFile(string filename, string encryptedFileName, DesEncryptOption option)
    {
    ...
    public void EncryptFile(string filename, Stream encryptedStream, DesEncryptOption option)
    {
