...
internal class CompoundDocument
{
    internal class StoragePart
    {
        ...
        internal Dictionary<string, StoragePart> SubStorage = new();
        internal Dictionary<string, byte[]> DataStreams = new();
    }

    internal StoragePart Storage;

    internal CompoundDocument() => Storage = new StoragePart();
    ...
    internal void Read(FileInfo fi) => Read(File.ReadAllBytes(fi.FullName));
    ...
    {
        using var doc = new CompoundDocumentFile();
    ...
    private void WriteStorageAndStreams(StoragePart storage, CompoundDocumentItem parent)
    {
        ...
        {
            var c = new CompoundDocumentItem() { Name = item.Key, ObjectType = 1, StreamSize = 0, Parent = parent };
        ...
        {
            var c = new CompoundDocumentItem() { Name = item.Key, ObjectType = 2, Stream = item.Value ?? Array.Empty<byte>(), StreamSize = item.Value == null ? 0 : item.Value.Length, Parent = parent };
            parent.Children.Add(c);
        }
    }
}
