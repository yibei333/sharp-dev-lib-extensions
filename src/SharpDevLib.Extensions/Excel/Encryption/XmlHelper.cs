...
internal abstract class XmlHelper
{
    ...
    /// </summary>
    internal enum InternalNodeInsertOrder
    {
        /// <summary>
    ...
    /// <returns></returns>
    internal XmlNode? CreateComplexNode(string path) => CreateComplexNode(TopNode, path, InternalNodeInsertOrder.SchemaOrder, null);
    ...
    /// <param name="path"></param>
    /// <returns></returns>
    internal XmlNode? CreateComplexNode(XmlNode? topNode, string path) => CreateComplexNode(topNode, path, InternalNodeInsertOrder.SchemaOrder, null);
    ...
    internal XmlNode? CreateComplexNode(XmlNode? topNode, string path, InternalNodeInsertOrder nodeInsertOrder, XmlNode? referenceNode)
    {
        if (string.IsNullOrWhiteSpace(path)) return topNode;
        ...
        foreach (var subPath in path.Split('/'))
        {
            ...
            if (subPath.Length > 0)
            {
                ...
                if (subPath.StartsWith("@"))
                {
                    ...
                    var attributeName = attributeSplit[0][1..];
                    ...
                    else
                    {
                        ...
                        if (attributeValue != null)
                        {
                            ...
                            {
                                node.Attributes[attributeName ?? string.Empty]!.Value = attributeValue;
                            }
                        }
                    }
                }
                else
                {
                    ...
                    if (subNode == null)
                    {
                        ...
                        if (nodeName.IndexOf("[") > 0)
                        {
                            ...
                            nodeName = nodeName[..nodeName.IndexOf("[")];
                        }
                        ...
                        {
                            if (node?.OwnerDocument != null && node.OwnerDocument.DocumentElement != null && node.OwnerDocument.DocumentElement.NamespaceURI == nameSpaceURI && node.OwnerDocument.DocumentElement.Prefix == string.Empty)
                            {
                                subNode = node.OwnerDocument.CreateElement(nodeName, nameSpaceURI);
                            }
                            else
                        ...
                        if (nodeInsertOrder == InternalNodeInsertOrder.SchemaOrder)
                        {
                            ...
                            if ((SchemaNodeOrder == null) || (SchemaNodeOrder.Length == 0))
                            {
                                ...
                                nodeInsertOrder = InternalNodeInsertOrder.Last;
                            }
                            else
                            {
                                ...
                                {
                                    nodeInsertOrder = InternalNodeInsertOrder.Before;
                                }
                                else
                                {
                                    nodeInsertOrder = InternalNodeInsertOrder.Last;
                        ...
                        if (subNode != null)
                        {
                            ...
                            {
                                case InternalNodeInsertOrder.After:
                                    node?.InsertAfter(subNode, referenceNode);
                                    referenceNode = null;
                                    break;

                                case InternalNodeInsertOrder.Before:
                                    node?.InsertBefore(subNode, referenceNode);
                                    referenceNode = null;
                                    break;

                                case InternalNodeInsertOrder.First:
                                    node?.PrependChild(subNode);
                                    break;

                                case InternalNodeInsertOrder.Last:
    ...
    internal int? GetXmlNodeIntNull(string path)
    {
        ...
        if (s != "" && int.TryParse(s, NumberStyles.Number, CultureInfo.InvariantCulture, out var i))
        {
            return i;
        }
        else
    ...
    {
        if (decimal.TryParse(GetXmlNodeString(path), NumberStyles.Any, CultureInfo.InvariantCulture, out var d))
        {
            return d;
        }
        else
